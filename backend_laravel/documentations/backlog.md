...
| Implementar UserController | profile (usuário autenticado) | [ ] |
| Implementar Progress/TrailUser | enroll, updateProgress, remove | [ ] |
| Validação | Form Requests opcionais | [ ] |
| Compatibilidade JSON | Respostas idênticas ao backend PHP antigo | [ ] |
| Middleware Sanctum | Proteger todas as rotas exceto login/register com `auth:sanctum` | [ ] |

### 6️⃣ Rotas da API
| Tarefa | Descrição | Status |
|--------|-----------|--------|
| Editar `routes/api.php` | Definir rotas de autenticação em `AuthController` (register/login/logout) | [*] |
| Incluir rotas de autenticação | `Route::post('/login', ...)` e `Route::post('/register', ...)` apontando para closures ou controllers simples que invocam o Fortify | [ ] |
| Grupo protegido | Aplicar `middleware('auth:sanctum')` | [ ] |
| Endpoints de recurso | Trilhas, cursos, perfil, progresso | [ ] |
| Compatibilidade de URL | Corresponder ao frontend | [ ] |
...
| Teste de logout | Verificar que `POST /logout` remove o token | [ ] |
...
| Testar conexão frontend | Executar `npm run dev` em frontend_react/ | [ ] |
| Ajustar BASE_URL se necessário | Atualizar `VITE_API_URL` no .env do frontend | [ ] |
| Validar formatos JSON | Confirmar estruturas de resposta idênticas | [ ] |
...
| Tarefa | Descrição | Status |
|--------|-----------|--------|
| Atualizar configuração do frontend | Apontar `VITE_API_URL` para a API Laravel | [ ] |
